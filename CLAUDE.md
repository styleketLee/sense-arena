...
- 앱명: 센스 아레나 (Sense Arena)
- 장르: 감각 능력 측정 테스트 시리즈
- 목표: 백엔드 없이 동작하는 MVP 구현
- 플랫폼: 토스 앱 내 WebView (iOS/Android)
- 수익: 리워드 광고 + 전면 광고 + 배너 광고
...
| 지점 | 형태 | 조건 |
|------|------|------|
| 결과 보기 전 | 리워드 광고 | "결과를 확인하려면 광고를 시청해주세요" |
| 테스트 사이 | 전면 광고 | 2개 테스트마다 1회 |
| 홈 하단 | 배너 광고 | 상시 노출 |
| 상세 결과 | 리워드 광고 | "더 자세한 분석 보기" |
...
│   ├── main.tsx               # React 루트
│   ├── router.tsx             # 라우팅
│   ├── domain/                # 순수 로직 (import 없는 순수 함수)
│   │   ├── types.ts           # 테스트 결과, 점수 타입
│   │   ├── constants.ts       # 테스트 설정값 모음
│   │   ├── scoring.ts         # 점수 산출 & 상위% 환산
│   │   └── testGenerators.ts  # 문제 생성 알고리즘 (색상, 패턴 등)
│   ├── stores/                # Zustand 상태 관리
│   │   ├── testStore.ts       # 진행 중 테스트 상태
│   │   └── recordStore.ts     # 최고 기록 & 히스토리
│   ├── storage/               # 저장 레이어
│   │   ├── storageAdapter.ts  # 앱인토스 Storage 래퍼
│   │   ├── schema.ts          # 저장 스키마 & 버전
│   │   └── migration.ts       # 마이그레이션
│   ├── pages/                 # 화면 컴포넌트
│   │   ├── HomePage.tsx       # 테스트 목록 & 내 기록 요약
│   │   ├── TestPage.tsx       # 테스트 진행 화면
│   │   ├── ResultPage.tsx     # 결과 & 공유 카드
│   │   └── RecordPage.tsx     # 전체 기록 히스토리
│   ├── tests/                 # 개별 테스트 컴포넌트
│   │   ├── ColorTest.tsx      # 색감 테스트
│   │   ├── ReactionTest.tsx   # 반응속도 테스트
│   │   ├── MemoryTest.tsx     # 순간기억력 테스트
│   │   └── AudioTest.tsx      # 청각 테스트
│   ├── components/            # 공통 컴포넌트
│   │   ├── ResultCard.tsx     # Canvas 결과 카드 렌더링
│   │   ├── ProgressBar.tsx    # 테스트 진행률
│   │   └── AdWrapper.tsx      # 광고 래퍼
│   └── bridge/                # 앱인토스 API 래퍼
│       └── appInToss.ts       # mock 분리 포함
├── public/
│   └── assets/
└── styles/
    └── global.css
